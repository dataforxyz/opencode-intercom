...
function isSessionRegistration(value) {
  ...
  const session = value;
  if (typeof session.cwd !== "string" || typeof session.model !== "string" || typeof session.pid !== "number" || typeof session.startedAt !== "number" || typeof session.lastActivity !== "number") {
    return false;
  }
  ...
  handleMessage(socket, msg, currentId, setId) {
    ...
    switch (clientMessage.type) {
      case "register": {
        ...
        setId(id);
        const info = { ...clientMessage.session, id };
